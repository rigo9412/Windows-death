        ...
        <div className="mb-8">
          ...
          <div className="text-sm text-blue-600 mb-4 flex justify-between items-center">
            ...
            <div className="space-x-2">
              <button
                onClick={exportLogs}
                className="px-4 py-1 bg-blue-900 hover:bg-blue-800 border border-blue-600 text-white transition-colors"
          ...
          <input
            ...
            className="w-full bg-gray-900 border border-blue-800 px-4 py-2 text-white placeholder-blue-800 focus:outline-none focus:border-blue-600"
          />
        </div>
        ...
        {filteredLogs.length === 0 ? (
          ...
          <div className="space-y-4">
            {filteredLogs.map((log, index) => (
              <div
                key={index}
                className="bg-gray-900 border border-blue-900 p-4 hover:border-blue-600 transition-colors"
              >
                <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
                  <div>
                    <span className="text-blue-600">TIMESTAMP:</span> {formatDate(log.timestamp)}
                  </div>
                  {log.utm_source && (
                    <div>
                      <span className="text-blue-600">SOURCE:</span> {log.utm_source}
                    </div>
                  )}
                  {log.utm_medium && (
                    <div>
                      <span className="text-blue-600">MEDIUM:</span> {log.utm_medium}
                    </div>
                  )}
                  {log.utm_campaign && (
                    <div>
                      <span className="text-blue-600">CAMPAIGN:</span> {log.utm_campaign}
                    </div>
                  )}
                  {log.utm_content && (
                    <div>
                      <span className="text-blue-600">CONTENT:</span> {log.utm_content}
                    </div>
                  )}
                  {log.utm_term && (
                    <div>
                      <span className="text-blue-600">TERM:</span> {log.utm_term}
                    </div>
                  )}
                  ...
                  <div className="md:col-span-2 text-xs text-blue-700">
                    <span className="text-blue-600">USER-AGENT:</span> {log.userAgent}
                  </div>
                </div>
              </div>
            ))}
